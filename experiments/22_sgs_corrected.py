"""
Exp 22 — multiple-testing-corrected recursive SGS (reviewer N1).

The plain recursive SGS (exp 20) over-splits partly because the per-node test
is repeated without correction. We apply a Bonferroni per-node level
alpha' = alpha / kcap and a matching n_null (so the small quantile is resolved),
and re-benchmark, to see whether the correction closes the gap with ESS-Dip.
"""
import os
...
N_REALIZ = 15
KCAP = 8
ALPHA = 0.05 / KCAP          # Bonferroni over the maximum number of splits
N_NULL, N_RUNS = 160, 2
...
def job(args):
    ci, cell, r = args
    rng = np.random.default_rng(50000 + ci * 100 + r)
    img, truth, _ = M.make_world(rng=rng, **cell)
    try:
        k = int(G.sgs_dip(img, rng, alpha=ALPHA, n_null=N_NULL,
                          n_runs=N_RUNS, kcap=KCAP))
    except Exception:
        k = -1
    return dict(kind=cell["kind"], truth_k=truth, method="SGS-Dip-mt", k_hat=k)


def main():
    cs = cells()
    jobs = [(i, c, r) for i, c in enumerate(cs) for r in range(N_REALIZ)]
    print(f"{len(jobs)} scenes, corrected SGS (alpha'={ALPHA:.4f}, B={N_NULL})")
    rows, done = [], 0
    ...
    with open(os.path.join(OUT, "sgs_mt_bench.csv"), "w", newline="") as fh:
    ...
    print(f"\nSGS-Dip-mt: spec {s:.2f} | struct {a:.2f} (MAE {mae:.2f}) | "
          f"mixed {c:.2f} | balanced {np.mean([s,a,c]):.3f}")
    print("  (plain SGS: 0.85|0.49|0.27|0.535   ESS-Dip: 1.00|0.81|0.50|0.771)")
